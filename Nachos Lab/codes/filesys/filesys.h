...
class FileSystem {
  public:
	...
	void WritePipe(char* data, int length); // write to pipe
	int ReadPipe(char* dst); // read fata from pipe and return length
	...
	int AddNewFile(OpenFile* file);
	bool RemoveFile(int openFileID);
	OpenFile* GetFile(int openFileID);
	void PrintTable();
	void CloseAll();
	bool CopyFile(int newTID);
